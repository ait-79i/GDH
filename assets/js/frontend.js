jQuery(document).ready(function ($) {
  ...
  function initFieldValidation() {
    ...
    $(document).on("input", ".gdh-rdv-field input", function () {
      ...
      if (
        $input.hasClass("gdh-rdv-field-error") ||
        $input.closest(".gdh-rdv-field").hasClass("has-error")
      ) {
        validateField($input);
      }
    });
    ...
    $(document).on(
      "change",
      '.gdh-rdv-checkbox-modern input[type="checkbox"]',
      function () {
  ...
  function validateField($input) {
    ...
    // Only validate format if there's a value and it passed the required check
    if (value && isValid) {
      // Check email format
      ...
      // Check pattern validation (for other input types)
      ...
      // Check minlength (runs after format checks)
      if (isValid && $input.attr("minlength")) {
  ...
  function validateCheckbox($checkbox) {
    ...
    const $errorMsg = $field.find(".gdh-rdv-error-message");
    const errorMessage =
      $checkbox.data("error-message") || "Vous devez accepter les conditions";
  ...
  function scrollToFirstError() {
    ...
    const $firstError = $step
      .find(".gdh-rdv-field-error, .gdh-rdv-checkbox-modern.has-error")
      .first();
  ...
  function initStep1Slots() {
    ...
    function createSlotCard(index) {
      ...
      try {
        if (
          window.Twig &&
          typeof window.Twig.twig === "function" &&
          window.gdhRdvData &&
          window.gdhRdvData.templates &&
          window.gdhRdvData.templates.slotCard
        ) {
          html = window.Twig.twig({
            data: window.gdhRdvData.templates.slotCard,
          }).render(context);
        }
      } catch (e) {
      ...
      const $card =
        html && typeof html === "string" && html.trim().length ? $(html) : null;
    ...
    function attachCardEvents($card) {
      ...
      $date.on("change", function () {
        ...
        if ($date.val()) {
          ...
          const hasTimeSelected =
            $card.find(".gdh-rdv-time.selected").length > 0 ||
      ...
      $allDay.on("click", function () {
        ...
        $allDay.attr(
          "aria-pressed",
          $allDay.hasClass("active") ? "true" : "false"
        );
    ...
    function updateCombinedValue($card) {
      ...
      if ($allDay.hasClass("active")) {
        ...
        const selectedTimes = $card
          .find(".gdh-rdv-time.selected")
          .map(function () {
    ...
    function clearCardError($card) {
      ...
      $card
        .find(".gdh-rdv-date, .gdh-rdv-times-grid")
        .removeClass("gdh-rdv-field-error");
      ...
      const remainingErrors = $step1.find(
        ".gdh-rdv-field-error, .gdh-rdv-slot-error"
      ).length;
    ...
    $cards.each(function (index) {
      ...
      if (isRequired) {
        if (!dateValue) {
          ...
          $card.append(
            '<div class="gdh-rdv-slot-error">Veuillez sélectionner une date</div>'
          );
        ...
        if (dateValue && !hasAllDay && !hasTimeSelected) {
          ...
          $card.append(
            '<div class="gdh-rdv-slot-error">Veuillez sélectionner au moins un horaire ou "Toute la journée"</div>'
          );
          if (!firstErrorCard) firstErrorCard = $card;
        }
      }
      ...
      if (!isRequired && hasData) {
        if (!dateValue) {
          ...
          $date.addClass("gdh-rdv-field-error");
          $card.append(
            '<div class="gdh-rdv-slot-error">Veuillez sélectionner une date pour cette disponibilité</div>'
          );
          if (!firstErrorCard) firstErrorCard = $card;
        }
